...
**인프라 모듈은 등록·적용 여부까지 확인**: 인프라 `@Module`/데코레이터/스케줄러는 정의뿐 아니라 (1) 엔트리(app.module 등)에 *등록*됐는지 (2) 실제 *사용처가 있는지* 를 grep 으로 확인해 "활성 / 등록만 / 미적용" 을 구분한다. 구현만 되고 안 쓰이는 것을 활성으로 보고하면 안 됨.
