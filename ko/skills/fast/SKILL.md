...
## 실행 방법
`${CLAUDE_PLUGIN_ROOT}/skills/plan/SKILL.md`를 **아래 오버라이드와 함께** 그대로 수행한다. 오버라이드하지 않은 섹션은 전부 원문 그대로 적용 — 모드 판별(§1), 진행 중 타 영역 요청(§1.5), build 전까지 문서만(§1.6), 병합·불필요 코드 확인(§1.7), 신규 모드(§3), 변경 모드(§4), 규약 영역 보강(§4.5), 보고(§5).
