---
name: fast
description: 기본 기획 진입점 — stick worktree 없이 현재 브랜치에서 바로 기능을 기획한다(spec → plan, 변경이면 revision). 문서가 현재 브랜치에 바로 쌓이고 이후 build/ship은 direct 모드로 동작. 워크트리 격리를 명시적으로 요구하지 않는 모든 기능 생성/수정/계획 요청에 사용. "기능 계획", "기능 생성", "기능 수정", "<기능명> 기획", "이 기능 만들어줘", "빠른 기획", "plan a feature", "fast <기능명>" 및 단계 명시 없는 맨몸 기획 요청에 발동. 신규/변경 자동 판별. analyze가 만든 CLAUDE.md 규약이 있어야 동작.
---

# fast — 워크트리 없는 기획, 현재 브랜치에서 바로 (기본 기획 진입점)

**beaver의 기본 기획 진입점이다** — 단계 명시 없는 맨몸 기능 요청은 여기로 온다; 워크트리 격리는 `/beaver:plan` 옵트인. 기획 절차는 `/beaver:plan`과 전부 동일하고, **stick worktree만 만들지 않는다**: 모든 작업이 현재 체크아웃된 브랜치, 현재 작업 디렉터리에서 일어난다. 이후 `/beaver:build`도 그 자리에서 누적하고, `/beaver:ship`은 **직접 모드** — 현재 브랜치에서 일반 커밋 + 푸쉬(병합·worktree 파기 없음)로 동작한다.

## 실행 방법
`${CLAUDE_PLUGIN_ROOT}/skills/plan/SKILL.md`를 **아래 오버라이드와 함께** 그대로 수행한다. 오버라이드하지 않은 섹션은 전부 원문 그대로 적용 — 모드 판별(§1), 진행 중 타 영역 요청(§1.5), build 전까지 문서만(§1.6), 병합·불필요 코드 확인(§1.7), 형제 코드 계약 준수(§1.8), 신규 모드(§3), 변경 모드(§4), 규약 영역 보강(§4.5), 보고(§5).
...
4. **plan §1.6이 여기서 더 강하게 적용된다**: 워크트리가 없어서 소스를 잘못 건드리면 작업 브랜치에 즉시 반영된다. `/beaver:build` 전까지는 문서만 쓴다 — 소스·테스트 코드는 절대 편집하지 않는다.
