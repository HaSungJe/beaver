...
build는 **커밋하지 않는다** — 구현만 하고 stick 브랜치(fast 직접 모드면 현재 브랜치)에 누적. 배포는 `/beaver:ship`.

## 0. stick 워크트리 보장 → memory 우선 + 모드·대상

**작업 위치 확정(어떤 읽기·쓰기보다 먼저)**: build는 plan(또는 fast)이 문서를 둔 곳 — stick 워크트리 또는 메인 체크아웃 — 에 누적한다. 틀린 쪽 금지. `.beaver/output/` 스캔이나 report 쓰기 전에 먼저 확인한다(안 그러면 새 세션이 report를 메인에 흘린다).
- **이미 stick worktree 안이면**(cwd가 `.claude/worktrees/<stick>` 이고 `.beaver/.auto-branch-state.json`에 키 존재) → 진행(워크트리 모드).
- **아니면(cwd가 메인)** — 먼저 **직접 모드(fast)** 확인: 메인 체크아웃의 `.beaver/output/`에 대상 기능의 미구현 plan/revision이 있으면(`/beaver:fast`가 생성) 현재 브랜치 **그 자리에서** 작업 — 워크트리 없음; 미커밋 누적하고 ship이 바로 커밋 + 푸쉬한다. `git branch --show-current` 비어 있으면 안 됨(detached → 중단).
- **그래도 아니면** → 메인을 더 스캔하지 **말 것**. `.claude/worktrees/` 아래에서 대상 기능의 미구현 plan/revision을 담은 stick 워크트리를 찾는다(기능명으로 매칭, stick 이름이 도메인 보유). 정확히 1개면 `EnterWorktree(name=<stick>)`로 재진입, 0개거나 2개+면 중단하고 `/beaver:plan`(또는 `/beaver:fast`) 먼저 돌리거나 대상을 명시하라고 안내.
- 같은 기능의 plan이 메인 체크아웃과 stick 워크트리 **양쪽에** 있으면 중단하고 어느 쪽을 build할지 사용자에게 묻는다.
...
## 3. 구현
`config.json` 경로 + `CLAUDE.md` 규약대로 plan/revision 설계를 구현해, 작성한 테스트 케이스를 충족시킨다. 변경 모드는 "변경 후 스펙"만 반영, 제거 분기 정리. plan/revision이 불필요로 표시한 기존 코드(plan §1.7)도 함께 삭제한다 — 추가는 그로 인한 삭제와 한 묶음으로 나간다.
