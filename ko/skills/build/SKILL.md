...
## 2. 테스트 먼저 (red)
plan/revision의 "테스트 케이스"를 **실제 테스트 코드로 먼저** 작성 — CLAUDE.md testing 규약 강도로(상태코드만 검증 금지). `commands.test_one`(`$NAME` 치환) 실행해 **의도대로 실패(red)** 하는지 확인(아직 구현 없음 → 실패가 정상). 컴파일이 막히면 시그니처/stub만 두어 red 상태를 확보. *테스트 저장 시 `self-heal` 훅이 자동 실행 — 첫 red는 정상이며, 이어서 구현으로 green을 만든다. 단 이 첫 red도 `.retry-count`를 1 소비하므로, 이후 green까지 남은 자동 재시도는 `self_heal_retry_limit - 1`회다.*

## 3. 구현 → green
`config.json` 경로 + `CLAUDE.md` 규약대로 plan/revision 설계를 구현해 테스트를 통과시킨다. 변경 모드는 "변경 후 스펙"만 반영, 제거 분기 정리.
- 실패 시 분석·수정·재실행(최대 `self_heal_retry_limit`, 기본 5). `self-heal` 훅은 **재시도 진행 중(`.retry-count` 존재)일 때만** 구현 파일 저장에서 자동 보조한다(§2의 첫 red가 `.retry-count`를 만든다).
