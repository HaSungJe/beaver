...
## 4. 보고 + 테스트 제안
커밋·리뷰·병합 결과 보고. 파괴 후 cwd는 `origin_branch`로 복귀 — remote가 있고 의존성이 설치된 상태, 즉 `/beaver:test`의 전제조건(test §0)과 정확히 일치. 따라서 **여기서 바로 사용자에게 회귀 테스트를 지금 실행할지 묻고**, 승인 시 같은 세션에서 즉시 `/beaver:test` 실행(이미 `origin_branch`에 있으므로 브랜치 전환 불필요):

- **예** → 지금 `/beaver:test` 실행(red 시 자가 치유 → 확인 후 수정 커밋/푸시).
- **아니오** → 안내만 남김: 나중에 `origin_branch`에서 `/beaver:test`로 배포 결과 검증.

그다음 다음 기능은 `/beaver:plan`.
