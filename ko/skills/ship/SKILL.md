...
**모드 판별**:
- **워크트리 모드** — cwd가 `.claude/worktrees/<stick>` 안이고 `.beaver/.auto-branch-state.json`에 해당 키 존재 → §1 리뷰 → §2 커밋 → §3 병합/복귀/파기.
...
**워크트리 모드 전용** — 직접 모드는 §3-direct로. §1 + §2 완료 후 진행. `origin_branch` = `.beaver/.auto-branch-state.json`의 stick 키 값. ship은 테스트를 돌리지 않는다 — ship 후 `origin_branch`에서 `/beaver:test`로 검증.
