...
**`.gitignore` 시드(필수, 멱등)** — 프로젝트 `.gitignore`에 아래 라인들을 보장한다(없는 것만 추가, 파일 없으면 생성, 있으면 넘어감):
```
...
```
beaver의 로컬 부기/설정 파일들이다(브랜치 상태, 머신별 커맨드·경로 config, stick 워크트리, plan §0의 `worktree.baseRef` 시드) — 커밋 대상이 아니다. **`.beaver/output/` 과 `.beaver/memory/` 는 추적 유지** — 문서와 memory는 브랜치와 함께 이동해 ship에서 병합된다. gitignore는 이미 추적 중인 파일에 영향 없음; 프로젝트가 `.claude/settings.json`을 의도적으로 버전 관리 중이면 강제하지 말고 사용자에게 알린다.

**`branch.stick_prefix`** — stick 브랜치/워크트리 접두(기본 `stick`). plan이 `<stick_prefix>/<domain>-<rand6>`으로 요청하면 하니스가 이름을 정규화하고(디렉터리 `.claude/worktrees/<접두>+<...>`, 브랜치 `worktree-<접두>+<...>`), plan은 **실제** 브랜치↔원래 브랜치 매핑을 `.beaver/.auto-branch-state.json`에 기록한다; 격리·병합·파기는 plan/ship 담당(→ ship §3). analyze는 이 값을 기록만 한다.
