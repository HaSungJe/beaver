...
**`paths.deps` + `commands.setup` (워크트리 의존성 주입)** — gitignore된 의존성 디렉터리(`node_modules`, `.venv`, `vendor`, …)는 fresh stick 워크트리에 체크아웃되지 **않아서**, 부팅 시 이를 해석하는 테스트 스위트는 진입 자체가 실패한다. `paths.deps`는 그 디렉터리들을 나열하고, plan §2가 워크트리 생성 직후 메인 repo의 것을 새 워크트리로 링크한다. `commands.setup`(예: `npm ci`, `pip install -r requirements.txt`)은 폴백 — 메인 repo에 dep 디렉터리가 없거나, stick이 링크된 디렉터리에 없는 새 의존성을 추가했을 때 워크트리 안에서 실행. 둘 다 스택에서 도출하고, 설치형 의존성 디렉터리가 없는 프로젝트면 `deps`(빈값/미설정)와 `setup`은 생략.
