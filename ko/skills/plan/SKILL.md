...
## 1.5 진행 중 타 영역 요청
이 흐름으로 기획·작업 진행 중에 다른 영역의 요청이 들어올 수 있다. 그 요청도 기획한다 — 미루지 않고 문서를 생성한다. 새 요청도 §1 모드 판별을 따로 거친다. 두 영역이 서로 맞닿거나 영향을 주면 작업 중이던 문서(spec/plan/revision)에 추가한다. 아니면 새 영역의 도메인 경로에 문서를 새로 만든다.
