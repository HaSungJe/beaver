...
## 1.6 build 전까지 문서만
여기서부터 `/beaver:build` 실행 전까지, 이 흐름에서 들어오는 모든 사용자 요청은 기획 요청이다 — 문서를 쓰거나 고치는 것으로 답하고, 소스·테스트 코드는 절대 편집하지 않는다. "X 추가해줘", "Y 지워줘", "Z 고쳐줘"처럼 직접 코드 작업으로 들리는 요청도 spec/plan/revision에만 반영하고 문서 변경으로 보고한다. 허용되는 쓰기는 기획 산출물뿐이다: `.beaver/output/` 문서, `.beaver/memory/`, §4.5 draft 규약 문서, §0/§2 관리 파일. 여기서 실제 코드를 고치면 plan/revision이 코드보다 이전 버전을 설명하게 된다; 문서가 항상 코드보다 최신이어야 한다.

## 1.7 병합·불필요 코드 확인 (코드 추가·삭제 설계 공통)
코드 추가를 설계하기 전에, 새 코드가 병합되어 들어갈 기존 코드 — 합류해야 하는 파일·유닛·패턴 — 를 먼저 찾고 그 근거(경로:라인) 위에서 연계를 설계한다. 이어서 그 추가·삭제로 불필요해지는 기존 코드가 있는지 확인한다 — 대체되는 분기, 더는 아무도 안 부르는 헬퍼, 안 쓰는 import, 죽은 설정 — 있으면 그 삭제를 plan/revision 변경 목록에 포함한다. 추가만 하고 대체한 것을 지우지 않는 설계는 미완이다.
