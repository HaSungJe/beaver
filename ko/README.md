...
코드베이스를 먼저 분석해 프로젝트 규약 문서(`CLAUDE.md`)를 만들고, 그 규약을 단일 근거로 **분석 → 기획 → 개발 → 배포(원래 브랜치 병합·푸쉬) → 리팩토링**을 일관되게 수행하는 Claude Code 플러그인. 기획·구현은 **기본적으로 현재 브랜치에서 바로**(`fast`) 이뤄지고, 선택적 **stick 워크트리 격리** 모드(`plan`)로 **여러 세션 병렬 작업**이 가능하다. 언어·프레임워크·포지션 무관 — 백엔드(NestJS · Spring · Python · Go · …), 프런트엔드(Next.js · React), 그리고 모바일 · CLI · 라이브러리 · 게임 엔진(Unity · Unreal · …) 프로젝트까지 동일하게.
...
- **다언어 지원** — 스택을 감지해 테스트·빌드 커맨드를 `.beaver/config.json` 에 기록하므로 언어에 비종속. 일반 매니페스트 밖 스택(Unity·Unreal 같은 게임 엔진)도 프로젝트 자체의 엔진/CI 파일에서 같은 방식으로 감지한다.
...
| 검증 (독립) | **테스트** | `/beaver:test` | "전체 테스트 돌려" | **전체 회귀**(`commands.test`) + 자가회복 — red면 소스를 자율 수정하고 green까지 재실행(가드 내), 이후 확인 후 커밋·푸쉬. green 후엔 **AI 동작 위험 점검**이 변경 표면에서 스위트가 못 보는 런타임 위험을 탐색(보고 전용). 독립 — 원격 있는 브랜치(ship 후 원래 브랜치)에서 돌리고, stick worktree 안에선 금지 |
...
- **실행 + 자가회복** — 현재 체크아웃에서 `commands.test` 실행 → **green**: 통과 보고; **red**: **소스**를 자율 수정하고(테스트 파일은 read-only — 사람이 승인한 판단 게이트만 테스트를 고칠 수 있음) 가드(maxAttempts, 진전 없음) 내에서 green까지 재실행, 이후 수정을 커밋하고 확인 후 푸쉬. 루프가 포기하면 어떤 테스트가 여전히 실패하는지 정확히 보고; 폴백 경로는 `/beaver:fast`(또는 plan)→`/beaver:build`, ship, 재실행.
- **동작 위험 점검 (green 후)** — AI가 **변경 표면**(변경 파일 + 직접 의존자; 요청 시 전체)을 탐색해 스위트가 못 보는 런타임 위험을 점검: 조용한 실패 경로, 배선 누락(작성됐지만 등록/라우팅 안 됨), mock 경계 너머 계약 드리프트, 상태·동시성 위험, 경계 동작. 발견은 코드로 검증(경로:라인 + 실패 시나리오)해 `.beaver/output/inspection/`에 기록하고 **보고만 — 자동 수정 금지**.
