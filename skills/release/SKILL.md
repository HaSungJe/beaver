...
## 0. 전제
- 로컬 `dam`(= `.beaver/config.json`의 `branch.integration`) 존재해야 함. 없으면 중단(먼저 `/beaver:plan`→`build`→`ship`).
...
- `templates/review.md` 기반 **`.beaver/output/review/dam-release-<YYMMDD>.md`** 작성. 같은 날 재리뷰면 `-<N>`.
