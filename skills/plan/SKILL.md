...
## 2. 브랜치 (선택)
- 현재 브랜치가 `.beaver/.auto-branch-state.json` 키면 질의 생략, 그대로 누적.
- 아니면 "새 작업 브랜치 만들까요?" 질의.
  - 예 → base = `branch.integration`(기본 `dam`). **로컬에 없으면 생성**: 원격 `origin/<base>` 있으면 추적 브랜치로, 없으면 메인라인에서 새로 만들고 알림. base에서 `<stick_prefix>/<domain>-<rand6>`(기본 `stick/...`) 생성·체크아웃 → state에 `{"<stick>": "<base>"}` 기록.
  - 아니오 → 현재 브랜치 유지.

## 3. 신규 모드
**spec** — `templates/spec.md` 기반 `.beaver/output/spec/<domain>/<feature>-spec.md`: 기능 설명 / API / 비즈니스 규칙 / 참고 + **확정 설계 결정사항**(CLAUDE.md만으로 못 정하는 항목을 `- [ ]`로, 답 비움). → 사용자가 답한 뒤 plan 지시.
