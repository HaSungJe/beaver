...
1. **Parallel deep analysis** — read the codebase quickly via fan-out (prefer parallel: Workflow parallel / Task distribution / sequential when not possible). Agents under `${CLAUDE_PLUGIN_ROOT}/agents/`:
   ...
   - test-pattern-analyzer — test conventions
   - reuse/adjacency scan (a general scan, not an agent) — similar features, reusable utils/services, DB schema/entities
2. **Classification** — determine whether the feature is ① an **addition to an existing pattern** (yet another CRUD, etc.) or ② **net-new** (a special area not present in the current code/conventions). If net-new, add a **technical implementation review + proposal** (required libraries/approaches/alternatives).
...
6. **Write plan** — plan document: file list / per-layer design / test cases / response codes + **prerequisite items** (`- [ ]` when infrastructure is absent; build is blocked until all are `[x]`). On save, the validator hook verifies it automatically.
