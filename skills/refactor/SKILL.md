...
## 4. 실행
실행 순서대로 작은 단위로: 공통 로직을 CLAUDE.md 규약 위치(전역/도메인 util, 공용 module)로 추출 → 호출부 교체(시그니처·네이밍 규약) → 죽은 코드 제거. **각 단계 후 테스트**, 깨지면 처리/되돌림. 계획서 체크박스 갱신.

## 5. 검증·보고
전체 `commands.test` 통과로 동작 보존 입증. 무엇을 어디로 묶었는지(규약 근거) 보고. 큰 건은 `/beaver:ship`에서 커밋 분리 제안.
