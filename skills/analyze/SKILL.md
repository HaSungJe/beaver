...
**`.gitignore` seed (required, idempotent)** — ensure the project's `.gitignore` contains the line `.beaver/.auto-branch-state.json` (internal plugin state: the session-local stick↔branch mapping — it must never be committed). Append it if missing; create `.gitignore` if the project has none; skip silently if already present. Only this state dotfile is ignored — `.beaver/config.json`, `output/`, and `memory/` stay tracked (they are project artifacts that ship with the work).
