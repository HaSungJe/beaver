...
```
`test_one`의 `$NAME`은 단일 테스트 실행 시 치환(pytest면 `pytest -k $NAME` 등). 커맨드는 사용자 확인.

**`branch.integration`는 항상 로컬 전용 일회용 통합 브랜치명(기본 `dam`)이다. mainline(`main`/`master` 등)으로 설정 금지** — mainline은 dam을 복제하는 *source*일 뿐이고 plan이 `.beaver/.dam-state.json`에 따로 기록한다. integration을 mainline으로 잡으면 dam 추상이 붕괴해 ship이 원격에 직접 발행하고 release의 `git branch -d <integration>`이 mainline을 삭제한다. 프로젝트가 main에 통합한다는 사실은 integration 값이 아니라 release의 source 선택으로 표현된다. 값을 생략하면 plan/ship이 `dam`으로 자동 생성한다.
