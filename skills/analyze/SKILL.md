...
**`branch.stick_prefix`는 stick 브랜치/워크트리 접두(기본 `stick`)다.** plan이 `<stick_prefix>/<domain>-<rand6>` 으로 `.claude/worktrees/`에 stick을 격리 생성한다. 별도 통합 브랜치 개념은 없다 — ship이 stick을 plan 시점의 원래 브랜치로 직접 병합·push한다.
