...
| Verify (independent) | **Test** | `/beaver:test` | "run the full tests" | Runs the **full regression** (`commands.test`) with self-heal — on red it autonomously fixes the source and re-runs until green (guarded), then commits and pushes after confirmation. After green, an **AI behavior-risk inspection** sweeps the change surface for runtime risks the suite cannot see (report-only). Standalone — run it on a branch with a remote (the original branch after ship), never inside a stick worktree |
...
ship           # one set · code review → commit → integrate origin into the stick (in the worktree)
               ...
               #   direct mode (fast flow): review → commit → push the current branch
...
- **Automatic stick worktree creation** — if already inside the current stick worktree, accumulate; otherwise, after recording `origin_branch = git branch --show-current`, use `EnterWorktree(name=stick/<domain>-<rand6>)` to create & enter an isolated worktree from the current HEAD (installing `worktree.baseRef=head` automatically). The harness normalizes names — actual dir `.claude/worktrees/stick+<...>`, actual branch `worktree-stick+<...>` — and plan records the **actual** `{stick branch: origin_branch}` in `.beaver/.auto-branch-state.json`. The original checkout stays on its branch by design — only the Claude session moves into the worktree.
...
- **Prerequisite** — a completed report or changes. **Mode detection**: inside a stick worktree (the current stick key exists in `.beaver/.auto-branch-state.json`) → worktree mode (full flow below); at the main checkout (fast flow) → **direct mode**: review → commit → integrate origin if remote tracking exists → `git push` the current branch. No merge into another branch, no worktree destroy.
...
- **Run + self-heal** — execute `commands.test` on the current checkout → **green**: report the pass; **red**: autonomously edit the **source** (test files are read-only — only the human-approved ASSESS gate may touch a test) and re-run until green within guards (maxAttempts, no-progress), then commit the fix and push after confirmation. If the loop gives up, report exactly which tests still fail; the fallback path is `/beaver:fast`(or plan)→`/beaver:build`, ship, re-run.
- **Behavior-risk inspection (after green)** — an AI pass explores the **change surface** (changed files + direct dependents; full project on request) for runtime risks the suite cannot see: silent failure paths, wiring gaps (written but never registered/routed), contract drift across mock boundaries, state/concurrency hazards, boundary behavior. Findings are verified against the code (path:line + failure scenario), written to `.beaver/output/inspection/`, and **reported only — never auto-fixed**.
...
├── skills/                  # 7 skills (slash + auto-trigger)
