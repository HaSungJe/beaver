...
- **회귀 방지** — 문서 구조 검증 + 테스트 자가수복(기본 5회, 막히면 plan으로 되먹임) + 병합 충돌 자동 해결. build는 그 기능 테스트만 보고, **전체 회귀는 원격 반영(release) 직전 1회** 돌려 속도와 안전망을 함께 챙긴다.
...
- **`ship` ↔ `resolve`** (세트) — 누적분을 커밋 → **코드 리뷰**(CLAUDE.md 규약 대비 + 의도 동작 확인) → `dam` 로컬 병합(모든 단계 승인 기반). stick·dam 모두 로컬 전용이라 ship은 원격에 push하지 않는다. 병합 충돌 시 `resolve` 가 ship 안에서 자동으로 통합한다.
