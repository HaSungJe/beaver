...
|---|---|---|---|---|
| 분석 (독립·1회) | **분석** | `/beaver:analyze` | "코드베이스 분석해줘" | 코드 정밀 분석 → `CLAUDE.md` + `docs/` 규약 + `.beaver/config.json` 생성 |
| 기획·구현 | **기획** | `/beaver:plan <기능명>` | "<기능명> 기획해줘" | 신규/변경 자동 판별 → spec → plan (또는 revision) 작성 |
| 기획·구현 | **개발** | `/beaver:build` | "작업 시작" | 구현 + 테스트 + 자가수복 + 리포트 (커밋 안 함) |
| 배포 | **배포** | `/beaver:ship` | "커밋하고 푸쉬", "dam에 병합" | 누적 작업 커밋 → 푸쉬 → dam 병합 (승인 기반) |
...
- **`analyze`** (독립) — 프로젝트당 한 번. 코드를 다각도(아키텍처·컨벤션·데이터·에러·API·테스트)로 분석해 규약 문서를 만든다.
- **`plan` → `build`** (세트) — 한 stick 브랜치에서 기능마다 반복. plan은 명세(spec→plan)를 정리하고, build는 구현·테스트·자가수복까지 하되 **커밋하지 않고** 변경을 쌓아둔다.
- **`ship` ↔ `resolve`** (세트) — 누적분을 커밋 → 푸쉬 → `dam` 병합(모든 단계 승인 기반). 병합 충돌 시 `resolve` 가 ship 안에서 자동으로 통합한다.
